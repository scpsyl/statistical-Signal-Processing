...
omega = 2 * pi * 100;     % 角频率，对应频率100Hz

% 时间序列
t = linspace(0, T, N);    % 时间向量，从0到T，共N个点
...
% 生成观测数据 z = A * cos(omega * t) + n
cos_wave = cos(omega * t);                  % 单周期的余弦信号
Z = A .* cos_wave + noise;                  % 每组数据包含N个采样

% 估计方法1：MLE估计
A_hat1 = mean(Z ./ cos_wave, 2);            % 最大似然估计

% 估计方法2：MAP估计
c = sigma_A_sq / (sigma_A_sq + sigma_n_sq / N);
A_hat_MAP = c * A_hat1;                     % MAP估计
...
% 输出结果
fprintf('实验：N=%d, 采样时间=%.1f秒, 噪声方差=%.2f\n\n', N, T, sigma_n_sq);
